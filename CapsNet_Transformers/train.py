...
class GestureTrainer(object):
    ...
    def init_model(self):
        ...
        self.train_loader = DataLoader(
            Dataset(self.data_path, split="train", data_type=self.data_type,number_of_labels=self.n_classes,
        ...
        self.val_loader = DataLoader(
            Dataset(self.data_path, split="val", data_type=self.data_type,number_of_labels=self.n_classes,
        ...
        if self.dataset == "shrec2017":
            ...
            self.test_loader = DataLoader(
                Dataset(self.data_path, split="test", data_type=self.data_type,number_of_labels=self.n_classes,
                        transforms=self.val_transforms, n_frames=self.clip_length, optical_flow=self.optical_flow,imsize=self.imsize),
    ...
    def __val(self):
        ...
        with torch.no_grad():
            for i, data_tuple in enumerate(tqdm(self.val_loader, desc="Val", postfix=""+str(np.random.randint(200)))):
                ...
                all_correct += int((predicted==correct).sum())
                all_values += int(correct.size(0))

                self.iters += 1
                self.update_metrics("val", loss.item(), inputs.size(0),
                                    float((predicted == correct).sum()) / len(correct))

        accuracy = all_correct/all_values
        self.tbx_summary.add_scalar('val_loss', self.losses["val"].avg, self.epoch + 1)
        self.tbx_summary.add_scalar('val_accuracy', accuracy, self.epoch + 1)
        print("Val Accuracy {}/{}: {}".format(all_correct,all_values,accuracy))
    ...
    def __test(self):
        ...
        self.tbx_summary.add_scalar('test_accuracy',accuracy, self.epoch + 1)
        print("Test Accuracy {}/{}: {}".format(all_correct,all_values,accuracy))
