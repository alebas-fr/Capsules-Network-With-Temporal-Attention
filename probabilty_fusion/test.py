...
from vote import VoteClassifier
...
class GestureTest(object):
    ...
    def __init__(self, configer):
        ...
        self.number_of_classifier = len(self.data_type)
        self.vote = VoteClassifier(self.number_of_classifier,self.n_classes,self.data_type)
    ...
    def init_model(self,data_type):
        """Initialize model and other data for procedure"""

        if self.optical_flow is True:
            self.in_planes = 2
        elif data_type in ["depth", "ir"]:
            self.in_planes = 1
        else:
            self.in_planes = 3

        # Selecting correct model and normalization variable based on type variable
        self.net = GestureTransoformer(self.configer.get("device"),self.configer.get("network",'backbone'),self.in_planes, self.n_classes,
        ...
        self.net, _, _, _ = self.model_utility.load_net(self.net,data_type=data_type)

        # Selecting Dataset and DataLoader
        if self.dataset == "briareo":
            Dataset = Briareo
            self.transforms = iaa.CenterCropToFixedSize(self.transforms_size[0],self.transforms_size[1])
        ...
        self.data_loader = DataLoader(
                Dataset(self.data_path, split="test", data_type=data_type,number_of_labels=self.n_classes,
                        transforms=self.transforms, n_frames=self.clip_length, optical_flow=self.optical_flow,imsize=self.imsize),
    ...
    def __test(self):
        ...
        while i<len(self.data_type):
            ...
            self.init_model(d_type)
            if i==0:
                self.vote.set_values(self.data_loader.__len__())
            self.net.eval()
            all_values = torch.zeros(self.data_loader.__len__(),self.n_classes)
            with torch.no_grad():
                for j,data_tuple in enumerate(tqdm(self.data_loader, desc="Test "+d_type)):
                    """
                    input, gt
                    """
                    ...
                    all_values[j] = output
            all_values = F.softmax(all_values,-1)
            self.vote.add_values_in_classifier(i,all_values)
            i+=1
        predicteds_values = self.vote.compute_score_classification()
        c = 0
        tot = 0
