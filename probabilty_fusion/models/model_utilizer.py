...
class ModuleUtilizer(object):
    ...
    def load_net(self, net,data_type):
        ...
        epoch = 0
        optimizer = None
        if self.configer.get("resume_"+data_type) != "None":
            print('Restoring checkpoint: ',self.configer.get("resume_"+data_type))
            checkpoint_dict = torch.load(self.configer.get("resume_"+data_type),map_location=self.device)
